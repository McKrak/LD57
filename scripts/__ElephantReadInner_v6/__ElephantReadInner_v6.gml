function __ElephantReadInner_v6(_buffer, _datatype)
{
    ...
    if (_datatype == buffer_array)
    {
        ...
        else
        {
            ...
            if (_length > 0)
            {
                ...
                repeat(_length)
                {
                    _array[@ _i] = __ElephantReadInner_v6(_buffer, _common_datatype);
                    ++_i;
                }
            }
    ...
    else if (_datatype == buffer_struct)
    {
        ...
        else if (_length == 0xFFFE) //Special value indicating that this is a struct created by a constructor
        {
            ...
            var _version_and_verbose = buffer_read(_buffer, buffer_u16);
            var _verbose = (_version_and_verbose >> 15);
            var _version = (_version_and_verbose & 0x7FFF);
            ...
            if (_verbose)
            {
                ...
                repeat(_length)
                {
                    ...
                    _struct[$ _name] = __ElephantReadInner_v6(_buffer, buffer_any);
                    ++_i;
                }
            }
            else
                ...
                if (is_struct(_elephantSchemas))
                {
                    ...
                    if (is_struct(_schema))
                    {
                        ...
                        repeat(array_length(_names))
                        {
                            ...
                            if (_name != undefined)
                            {
                                _struct[$ _name] = __ElephantReadInner_v6(_buffer, _schema[$ _name]);
                            }
    ...
    else if (_datatype == buffer_any)
    {
        ...
        return __ElephantReadInner_v6(_buffer,_datatype);
    }
