...
function VBM_Animation_BakeAnimationLocal(animation) {
	...
	for (var frame = duration-1; frame >= 0; frame--) {
		...
		repeat(curve_count) {
			...
			_sy = transforms[curve_index+VBM_T_SCALEY];
		    ...
		    mA[VBM_M10] = (2.0 * (xy + zw)) * _sy;
		    mA[VBM_M11] = (1.0 - 2.0 * (xx + zz)) * _sy;
		    mA[VBM_M12] = (2.0 * (yz - xw)) * _sy;
...
function VBM_Model_Clear(vbmmodel) {
	...
	for (var animation_index = 0; animation_index < n; animation_index++) {
		var _ani = vbmmodel.animations[animation_index];
        VBM_Animation_Free(_ani);
	}
...
function VBM_Model_SampleAnimation_Mat4(vbmmodel, animation, frame, outmat4arrayflat) {
	...
	var t;
	var qw, qx, qy, qz;
	var xx, xy, xz, xw, yy, yz, yw, zz, zw, sx, _sy, sz;
	...
	repeat(bone_count-1) {
		...
		_sy = outmat4arrayflat[bone_index+VBM_T_SCALEY];
...
function VBM_Animator_SetRootTransform(animator, x,y,z, radiansx,radiansy,radiansz, sx,_sy,sz) {
	animator.transforms[VBM_T_SCALEX] = sx;
	animator.transforms[VBM_T_SCALEY] = _sy;
		...
		while (bone_index < bone_count) {
			...
			repeat(swing_count) {
				if ( bone_hash == animator.swing_bones[swing_index].bone_hash ) {
					...
					cz = cos(yawpt5);
					sx = sin(rollpt5);
					_sy = 0.0; // sin(pitchpt5);
					sz = sin(yawpt5);
					
					transforms[@ transform_offset + VBM_T_QUATW] = cx*cy*cz - sx*_sy*sz;
					transforms[@ transform_offset + VBM_T_QUATX] = sx*cy*cz + cx*_sy*sz;
					transforms[@ transform_offset + VBM_T_QUATY] = cx*_sy*cz - sx*cy*sz;
					transforms[@ transform_offset + VBM_T_QUATZ] = cx*cy*sz + sx*_sy*cz;
					break;
				}
				swing_index += 1;
			}
			...
			_sy = transforms[transform_offset+VBM_T_SCALEY];
...
function VBM_OpenVBM(fpath, outvbm, flags=0) {
	var b;
	if (flags && VBM_OPENFLAGS.FROM_BUFFER) {
		b = fpath;
	} else {
		b = buffer_load(VBM_PROJECTPATH+fpath);
	}
	...
	for (var res_index = 0; res_index < rescount; res_index++) {
		...
		if (restypestr == "TEX") {
			...
			outvbm.texture_sprites[texture_index] = word;
			texture_index += 1;
			//surface_free(surf);
		...
		// Mesh .......................................................
			...
			if ( (flags & VBM_OPENFLAGS.NOFREEZE) == 0 ) {
				vertex_freeze(vb); 
                //BUG: Vertex freezing temporairly unsupported (causes memory leak)
			}
...
function __vbm_mat4compose(outmat4, x,y,z, qw,qx,qy,qz, sx,_sy,sz) {
	...
	outmat4[@ VBM_M03] = x;
	outmat4[@ VBM_M10] = (2.0 * (xy + zw)) * _sy;
	outmat4[@ VBM_M11] = (1.0 - 2.0 * (xx + zz)) * _sy;
	outmat4[@ VBM_M12] = (2.0 * (yz - xw)) * _sy;
	outmat4[@ VBM_M13] = y;
